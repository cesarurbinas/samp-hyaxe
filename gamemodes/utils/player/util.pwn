...
public CheckFish(playerid)
{
	if(PLAYER_TEMP[playerid][py_FISHING])
	{
		...
		if(PLAYER_TEMP[playerid][py_FISHING_PROGRESS] > 50)
		{
			...
			PLAYER_SKILLS[playerid][WORK_FISHER] ++;
		}
		else
		{
...
CheckClubOptions(playerid)
{
	if (PLAYER_TEMP[playerid][py_CLUB_INDEX] != -1)
	{
		...
		if (GetPlayerInterior(playerid) != 0 && GetPlayerInterior(playerid) != 6)
		{
...
BuyProperty(playerid, id)
{
	...
	if (PROPERTY_INFO[index][property_EXTRA] > ACCOUNT_INFO[playerid][ac_SD]) return ShowPlayerMessage(playerid, "~r~No tienes las vulcoins suficientes.", 3);
