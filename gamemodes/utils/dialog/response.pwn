public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	switch(dialogid)
	{
		...
		case DIALOG_REGISTER_EMAIL:
		{
			...
			if(email_exists)
			{
			    ShowPlayerMessage(playerid, "~r~Este correco electrónico ya está en uso, use otro distinto.", 3);
		...
		case DIALOG_LOGIN:
		{
			...
			if(!strcmp(password, ACCOUNT_INFO[playerid][ac_PASS], false))
			{
				...
				hy_ShowNotification(playerid, str_text, 4);

				if (PLAYER_MISC[playerid][MISC_CONFIG_LOWPC] == true) SetPlayerWeather(playerid, 32);
    			else SetPlayerWeather(playerid, SERVER_WEATHER);
			}
			else // Error
			{
				PLAYER_TEMP[playerid][py_BAD_LOGIN_ATTEMP] ++;
				if(PLAYER_TEMP[playerid][py_BAD_LOGIN_ATTEMP] > MAX_BAD_LOGIN_ATTEMPS) return Kick(playerid);
				ShowDialog(playerid, dialogid);
			}
			return 1;
		}
		...
		case DIALOG_PLAYER_TOYS:
		{
			if(response)
			{
				...
				{
					SendClientMessageEx(playerid, COLOR_WHITE, ""COL_WHITE"¡Los jugadores una {b1fc6f}Tarjeta VIP "COL_WHITE"pueden tener hasta %d accesorios! Usa {fffb49}/ayuda "COL_WHITE"si quieres tener una {b1fc6f}Tarjeta VIP.", MAX_PLAYER_ATTACHED_OBJECTS);
					ShowDialog(playerid, dialogid);
					return 1;
				}
				...
				switch(listitem)
				{
					case 0: //colocar / quitar
					{
						if(PLAYER_TOYS[playerid][ PLAYER_TEMP[playerid][py_SELECTED_TOY_SLOT] ][player_toy_ATTACHED]) // tiene accesorio puesto
						{
							...
							new action[45]; format(action, sizeof action, "se quita su \"%s\"", PLAYER_TOYS[playerid][ PLAYER_TEMP[playerid][py_SELECTED_TOY_SLOT] ][player_toy_NAME]);
							//Auto_SendPlayerAction(playerid, action);
						...
						else // No lo tiene puesto
						{
							...
							new action[45]; format(action, sizeof action, "se coloca su \"%s\"", PLAYER_TOYS[playerid][ PLAYER_TEMP[playerid][py_SELECTED_TOY_SLOT] ][player_toy_NAME]);
							//Auto_SendPlayerAction(playerid, action);
							SetPlayerChatBubble(playerid, "\n\n\n\n* Se coloca algo.\n\n\n", 0xffcb90FF, 20.0, 5000);
						}
					}
		...
		case DIALOG_PLAYER_TOY_COLOR_1:
		{
			if(response)
			{
				...
				else PLAYER_TOYS[playerid][ PLAYER_TEMP[playerid][py_SELECTED_TOY_SLOT] ][player_toy_COLOR_1] = RGBAToARGB(RandomColors[listitem]);
		...
		case DIALOG_PLAYER_TOY_COLOR_2:
		{
			if(response)
			{
				...
				else PLAYER_TOYS[playerid][ PLAYER_TEMP[playerid][py_SELECTED_TOY_SLOT] ][player_toy_COLOR_2] = RGBAToARGB(RandomColors[listitem]);
				...
				else
				{
					...
					if(PLAYER_PHONE[transfer_player_id][player_phone_VALID])
					{
						...
						if(PLAYER_PHONE[transfer_player_id][player_phone_STATE] == PHONE_STATE_ON)
						{
							...
							{
								SendClientMessageEx(transfer_player_id, COLOR_WHITE, "{E8FF00}[Nuevo mensaje recibido] "COL_WHITE"%s: %s", convertPhoneNumber(transfer_player_id, 6740), message);
							}
						}
					}
				}
				...
				switch(Supermarket_Product_List[listitem][product_TYPE])
				{
					case PRODUCT_TYPE_FOOD:
					{
						if(CHARACTER_INFO[playerid][ch_CASH] >= Supermarket_Product_List[listitem][product_PRICE])
						{
							...
							{
								PlayerPlaySoundEx(playerid, 1085, 0.0, 0.0, 0.0);
						        ShowPlayerMessage(playerid, "~r~No tienes más espacio entre tus alimentos, elimina algo con /alimentos.", 3);
								return 1;
							}*/
							...
							SendClientMessageEx(playerid, COLOR_WHITE, "Has comprado "COL_RED"%s "COL_WHITE"por "COL_RED"%d$, "COL_WHITE"para consumirlo usa "COL_RED"/alimentos.", Supermarket_Product_List[listitem][product_NAME], Supermarket_Product_List[listitem][product_PRICE]);
							ApplyAnimation(playerid, "DEALER", "SHOP_PAY", 4.1, false, false, false, false, 0, false); // pagar*/

							new action[64];
							format(action, sizeof action, "compra %s y lo consume.", Supermarket_Product_List[listitem][product_NAME]);
							SetPlayerChatBubble(playerid, "\n\n\n\n* Consume comida.\n\n\n", 0xffcb90FF, 20.0, 5000);
							ShowPlayerNotification(playerid, "Consumiendo...", 3);
			...
			if(response)
			{
				switch(listitem)
				{
					case 0: //Consumir
					{
						...
						new action[64];
						format(action, sizeof action, "consume %s.", PLAYER_POCKET[playerid][PLAYER_TEMP[playerid][py_POCKET_SLOT_SELECTED]][player_pocket_object_NAME]);
						//Auto_SendPlayerAction(playerid, action);
						SetPlayerChatBubble(playerid, "\n\n\n\n* Consume algo.\n\n\n", 0xffcb90FF, 20.0, 5000);
					...
					case 0: //llamar
					{
						...
						switch(PLAYER_PHONE_BOOK[playerid][ PLAYER_TEMP[playerid][py_PLAYER_PHONE_BOOK_SELECTED] ][phone_book_contact_PHONE_NUMBER])
						{
							case 911:
							{
								...
								SendClientMessage(playerid, COLOR_WHITE, ""COL_WHITE"{cde943}Operadora:"COL_WHITE" Ha llamado a la central de policía, indícanos su nombre por favor.");
								return 1;
							}
							case 570: //Mecanico
							{
						...
						SendClientMessageEx(to_playerid, COLOR_WHITE, "{E8FF00}[Llamada entrante] "COL_WHITE"%s te está llamando, para responser usa {E8FF00}/responder "COL_WHITE"o {E8FF00}/colgar.", convertPhoneNumber(to_playerid, PLAYER_PHONE[playerid][player_phone_NUMBER]));
						...
						SendClientMessageEx(playerid, COLOR_WHITE, "Llamando a {E8FF00}%s ...", PLAYER_PHONE_BOOK[playerid][ PLAYER_TEMP[playerid][py_PLAYER_PHONE_BOOK_SELECTED] ][phone_book_contact_NAME]);
		...
		case DIALOG_PHONE_CALL_NUMBER:
		{
			if(response)
			{
				...
				SendClientMessageEx(playerid, COLOR_WHITE, "Llamando a {E8FF00}%s ...", convertPhoneNumber(playerid, inputtext[0]));
				ShowPlayerMessage(playerid, "Llamando...", 2);
				PLAYER_TEMP[playerid][py_TIMERS][6] = SetTimerEx("NoCallResponse", 15000, false, "i", playerid);
				PLAYER_TEMP[playerid][py_PLAYER_IN_CALL] = true;
				PLAYER_TEMP[playerid][py_PLAYER_PHONE_CALL_STATE] = CALL_STATE_WAITING_RESPONSE;
		...
		case DIALOG_PHONE_SMS_MESSAGE:
		{
			if(response)
			{
				...
				else
				{
					...
					if(PLAYER_PHONE[to_playerid][player_phone_STATE] == PHONE_STATE_ON) SendClientMessageEx(to_playerid, COLOR_WHITE, "{E8FF00}[Nuevo mensaje recibido] "COL_WHITE"%s: %s", convertPhoneNumber(to_playerid, PLAYER_PHONE[playerid][player_phone_NUMBER]), inputtext);
					PlayAudioStreamForPlayer(to_playerid, "https://www.dl.dropboxusercontent.com/s/do1ntkjac2kb7qn/ntf.mp3");
				}
		...
		case DIALOG_CONFIRM_BUY_PROPERTY:
		{
			if(response)
			{
				...
				if(PROPERTY_INFO[PLAYER_TEMP[playerid][py_BUY_HOUSE_INDEX]][property_VIP_LEVEL] > ACCOUNT_INFO[playerid][ac_SU]) return ShowPlayerMessage(playerid, "~r~No tienes una Tarjeta VIP", 2);
		...
		case DIALOG_PLAYER_GPS_VEHICLES:
		{
			if(response)
			{
				...
				if(!ACCOUNT_INFO[playerid][ac_SU])
				{
					...
					{
					    ShowPlayerMessage(playerid, "~r~No podrás usar este vehículo hasta que no consigas una Tarjeta VIP.", 3);
					}
				}
		...
		case DIALOG_GASOLINE:
		{
			if(response)
			{
				switch(listitem)
				{
					case 0: ShowDialog(playerid, DIALOG_LOAD_GASOLINE);
					case 1:
					{
				...
				switch(listitem)
				{
					...
					/*case 2:
					{
						...
						for(new i = 0; i != MAX_TERRITORIES; i ++)
						{
							...
							if(IsPointInDynamicArea(TERRITORIES[i][territory_AREA], PROPERTY_INFO[ PLAYER_TEMP[playerid][py_PLAYER_PROPERTY_SELECTED] ][property_EXT_X], PROPERTY_INFO[ PLAYER_TEMP[playerid][py_PLAYER_PROPERTY_SELECTED] ][property_EXT_Y], PROPERTY_INFO[ PLAYER_TEMP[playerid][py_PLAYER_PROPERTY_SELECTED] ][property_EXT_Z]))
							{
								PLAYER_TEMP[playerid][py_PLAYER_TERRITORY_PRO] = i;
								ShowDialog(playerid, DIALOG_CREW_PROPERTY_CONFIRM);
								return 1;
							}
						}
						ShowPlayerMessage(playerid, "~r~Esta propiedad no está dentro de un territorio de tu crew.", 3);
					}*/
				}
			}
			return 1;
			...
			if(response)
			{
				...
				else
				{
					if(PLAYER_PHONE[playerid][player_phone_VALID])
					{
						...
						RegisterPhoneMessage(6740, PLAYER_PHONE[playerid][player_phone_NUMBER], message);
						if(PLAYER_PHONE[playerid][player_phone_STATE] == PHONE_STATE_ON) SendClientMessageEx(playerid, COLOR_WHITE, "{E8FF00}[Nuevo mensaje recibido] "COL_WHITE"%s: %s", convertPhoneNumber(playerid, 6740), message);
						PlayAudioStreamForPlayer(playerid, "https://a.tumblr.com/tumblr_pmsy3s8hQ21xhjynto1.mp3");
		...
		case DIALOG_PROPERTY_SELL_TO_PLAYER:
		{
			if(response)
			{
				...
				new action[64]; format(action, sizeof action, "quiere llegar a un acuerdo con %s.", PLAYER_TEMP[inputtext[0]][py_RP_NAME]);
				//Auto_SendPlayerAction(playerid, action);
		...
		case DIALOG_SELL_CONFIRM_PROPERTY:
		{
			if(response)
			{
				...
				new action[64]; format(action, sizeof action, "y %s han llegado a un acuerdo.", PLAYER_TEMP[seller][py_RP_NAME]);
				//Auto_SendPlayerAction(playerid, action);
		...
		case DIALOG_SELL_CONFIRM_VEHICLE:
		{
			if(response)
			{
				...
				if(PLAYER_PHONE[seller][player_phone_VALID])
				{
					...
					RegisterPhoneMessage(6740, PLAYER_PHONE[seller][player_phone_NUMBER], message);
					if(PLAYER_PHONE[seller][player_phone_STATE] == PHONE_STATE_ON) SendClientMessageEx(seller, COLOR_WHITE, "{E8FF00}[Nuevo mensaje recibido] "COL_WHITE"%s: %s", convertPhoneNumber(seller, 6740), message);
					PlayAudioStreamForPlayer(playerid, "https://www.dl.dropboxusercontent.com/s/do1ntkjac2kb7qn/ntf.mp3");
				}
		...
		case DIALOG_SEED_LIST:
		{
			if(response)
			{
				if(seed_info[listitem][seed_info_EXP] > PLAYER_SKILLS[playerid][WORK_FARMER])
				{
					new str_text[128];
					format(str_text, sizeof(str_text), "Necesitas plantar %d plantas para comprar esta semilla", seed_info[listitem][seed_info_EXP]);
					return ShowPlayerNotification(playerid, str_text, 3);
				}
		...
		case DIALOG_SEED_BUY:
		{
			if(response)
			{
				...
				GivePlayerCash(playerid, -price);
				ShowPlayerNotification(playerid, "Semillas compradas.", 3);
			}
		...
		case DIALOG_PLANT_PLANTS:
		{
			if(response)
			{
				...
				SendClientMessageEx(playerid, COLOR_WHITE, ""COL_WHITE"Vas a plantar '%s' y vas a necesitar %d semillas.", plant_info[listitem][plant_info_NAME], plant_info[listitem][plant_info_SEEDS]);
				...
				PLAYER_TEMP[playerid][py_WORKING_IN] = WORK_FARMER;
				PLAYER_TEMP[playerid][py_PLANTING_PROGRESS] = frandom(10.0, 8.0, 2) + offset;
			...
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						...
						//if(!PLAYER_CREW[playerid][player_crew_VALID] && !ACCOUNT_INFO[playerid][ac_SU]) return ShowPlayerMessage(playerid, "~r~Solo puedes comprar armas aquí si perteneces a una crew o tienes una Tarjeta VIP.", 3);
		...
		case DIALOG_BLACK_MARKET_WEAPONS:
		{
			if(response)
			{
				if(CHARACTER_INFO[playerid][ch_CASH] >= BLACK_MARKT_WEAPONS[listitem][black_market_WEAPON_PRICE])
				{
					...
					if(WEAPON_INFO[ BLACK_MARKT_WEAPONS[listitem][black_market_WEAPON_ID] ][weapon_info_AMMO]) GivePlayerWeaponEx(playerid, BLACK_MARKT_WEAPONS[listitem][black_market_WEAPON_ID], 2000);
					else GivePlayerWeaponEx(playerid, BLACK_MARKT_WEAPONS[listitem][black_market_WEAPON_ID], 2000);

					if(BLACK_MARKT_WEAPONS[listitem][black_market_WEAPON_ID] == 41)
	            	{
	            		GivePlayerWeaponEx(playerid, BLACK_MARKT_WEAPONS[listitem][black_market_WEAPON_ID], 9999);
	            	}
			...
			if(response)
			{
				switch(listitem)
				{
					...
					case 2:
					{
						...
						GivePlayerWeaponEx(playerid, PLAYER_WEAPONS[playerid][ PLAYER_TEMP[playerid][py_SELECTED_DIALOG_WEAPON_SLOT] ][player_weapon_ID], 150);

						format(str_text, sizeof(str_text), "Compraste 150 balas para tu %s.", WEAPON_INFO[ PLAYER_WEAPONS[playerid][ PLAYER_TEMP[playerid][py_SELECTED_DIALOG_WEAPON_SLOT] ][player_weapon_ID] ][weapon_info_NAME]);
						ShowPlayerNotification(playerid, str_text, 3);
		...
		case DIALOG_ANIMS:
		{
		    if(response)
		    {
			   ...
			   ShowPlayerMessage(playerid, "Para detener la animacion usa /parar", 2);
			}
			return 1;
		}
		case DIALOG_TRICKS_FOOD:
		{
			if(response)
			{
				...
				new action[64];
				format(action, sizeof action, "y %s llegan a un acuerdo.", PLAYER_TEMP[playerid][py_RP_NAME]);
				Auto_SendPlayerAction(PLAYER_TEMP[playerid][py_TRICK_SELLER_PID], action);
			...
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						...
						{
							PlayerPlaySoundEx(playerid, 1085, 0.0, 0.0, 0.0);
							SendClientMessageEx(playerid, COLOR_WHITE, "Para sacar esta arma tienes que deshacerte de tu '%s' (%d) para tener espacio.", WEAPON_INFO[ PLAYER_WEAPONS[playerid][ to_slot ][player_weapon_ID] ][weapon_info_NAME], to_slot);
							return 1;
						}
		...
		case DIALOG_SHOP_TOYS:
		{
			if(response)
			{
				if(CHARACTER_INFO[playerid][ch_CASH] >= Toys_Shop[listitem][shop_toy_price])
				{
					...
					{
						PlayerPlaySoundEx(playerid, 1085, 0.0, 0.0, 0.0);
						ShowPlayerMessage(playerid, "~r~No tienes más espacio para accesorios, elimina alguno si quieres comprar otro~n~o adquiere membresia VIP.", 5);
						SendClientMessageEx(playerid, COLOR_WHITE, ""COL_WHITE"¡Los jugadores con una {b1fc6f}Tarjeta VIP "COL_WHITE"pueden tener hasta %d accesorios! Usa {fffb49}/ayuda "COL_WHITE"si quieres tener una {b1fc6f}Tarjeta VIP.", MAX_PLAYER_ATTACHED_OBJECTS);
						return 1;
					}
		...
		case DIALOG_CRANE_SELECT_VEHICLE:
		{
			if(response)
			{
				...
				{
					SendClientMessageEx(playerid, COLOR_WHITE, "Necesitas %s$ para remolcar este vehículo al depósito municipal.", veh_money);
					return 1;
				}
				...
				SendClientMessageEx(playerid, COLOR_WHITE, ""COL_WHITE"Tu vehículo %s ha sido remolcado hasta el depósito municipal.", VEHICLE_INFO[GLOBAL_VEHICLES[ PLAYER_TEMP[playerid][py_PLAYER_LISTITEM][listitem] ][gb_vehicle_MODELID] - 400][vehicle_info_NAME]);
			}
			return 1;
		}
		...
		case DIALOG_SEND_REPORT:
		{
			if(response)
			{
				...
				new str[145]; format(str, 145, "{e5580d}[REPORTE] {e5d10d}%s (%d) > %s (%d): %s", ACCOUNT_INFO[playerid][ac_NAME], playerid, ACCOUNT_INFO[ PLAYER_TEMP[playerid][py_LAST_TARGET_PLAYER] ][ac_NAME], PLAYER_TEMP[playerid][py_LAST_TARGET_PLAYER], reason);
		...
		case DIALOG_CREATE_CREW_CONFIRM:
		{
			if(response)
			{
				...
				CREW_INFO[index][crew_COLOR] = RandomColors[ PLAYER_TEMP[playerid][py_CREATE_CREW_COLOR] ];
			...
			if(response)
			{
				...
				switch(PLAYER_TEMP[playerid][py_PLAYER_LISTITEM][listitem])
				{
					...
					/*case 2:
					{
						...
						format(caption, sizeof caption, "{%06x}%s", CREW_INFO[ PLAYER_CREW[playerid][player_crew_INDEX] ][crew_COLOR] >>> 8, CREW_INFO[ PLAYER_CREW[playerid][player_crew_INDEX] ][crew_NAME]);
					    format(dialog, sizeof dialog, ""COL_WHITE"Los graffitis son cada 45 minutos, han pasado %s minutos desde el último.", TimeConvert(time));
					    ShowPlayerDialog(playerid, DIALOG_INFO, DIALOG_STYLE_MSGBOX, caption, dialog, "Cerrar", "");
					}*/
		...
		case DIALOG_CREW_RANK_MODIFY_NAME:
		{
			...
			if(response)
			{
				...
				SendClientMessageEx(playerid, COLOR_WHITE, "Has modificado el nombre del rango '%s' a '%s'.", CREW_RANK_INFO[ PLAYER_CREW[playerid][player_crew_INDEX] ][ PLAYER_TEMP[playerid][py_CREW_SELECTED_RANK] ][crew_rank_NAME], rank_name);
		...
		case DIALOG_CREW_RANK_CREATE:
		{
			...
			if(response)
			{
				...
				SendClientMessageEx(playerid, COLOR_WHITE, "El rango '%s' ha sido creado, ahora puedes editarlo y darle permisos.", rank_name);
		...
		case DIALOG_CREW_DELETE:
		{
			...
			if(response)
			{
				...
				for(new i = 0; i != MAX_TERRITORIES; i ++)
				{
					...
					TERRITORIES[ i ][territory_COLOR] = 0xF7F7F755;
					UpdateGangZoneColor(i);
				}
		...
		case DIALOG_CREW_MODIFY_MEMBER:
		{
			...
			if(response)
			{
				...
				format(message, sizeof message, "Has modificado el rango de '%s' al rango '%s'.", name, CREW_RANK_INFO[ PLAYER_CREW[playerid][player_crew_INDEX] ][ PLAYER_TEMP[playerid][py_PLAYER_LISTITEM][listitem] ][crew_rank_NAME]);
		...
		case DIALOG_CREW_LEAVE_TERRITORY:
		{
			...
			if(response)
			{
				...
				TERRITORIES[ PLAYER_TEMP[playerid][py_PLAYER_TERRITORY_PRO] ][territory_COLOR] = 0xF7F7F755;
				...
				new message[145];
				format(message, sizeof message, "{f4e242}[CREWS] '%s' ha abandonado un territorio en %s.", CREW_INFO[ PLAYER_CREW[playerid][player_crew_INDEX] ][crew_NAME], TERRITORIES[ PLAYER_TEMP[playerid][py_PLAYER_TERRITORY_PRO] ][territory_NAME]);
				for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
				{
					if(IsPlayerConnected(i))
					{
		...
		case DIALOG_VOBJECT_MENU:
		{
			if(response)
			{
				switch(listitem)
				{
					...
					case 1: //Comprar
					{
						...
						{
							PlayerPlaySoundEx(playerid, 1085, 0.0, 0.0, 0.0);
							SendClientMessageEx(playerid, COLOR_WHITE, ""COL_WHITE"¡Los jugadores con una {b1fc6f}Tarjeta VIP "COL_WHITE"pueden tener hasta %d objetos en vehículos! Usa {fffb49}/ayuda "COL_WHITE"si quieres tener una {b1fc6f}Tarjeta VIP.", MAX_SU_VOBJECTS);
							ShowDialog(playerid, dialogid);
							return 1;
						}
						if(slot == -1)
					...
					case 2:
					{
						...
						{
							PlayerPlaySoundEx(playerid, 1085, 0.0, 0.0, 0.0);
							SendClientMessageEx(playerid, COLOR_WHITE, ""COL_WHITE"¡Los jugadorescon una {b1fc6f}Tarjeta VIP "COL_WHITE"pueden tener hasta %d objetos en vehículos! Usa {fffb49}/ayuda "COL_WHITE"si quieres ser una {b1fc6f}Tarjeta VIP.", MAX_SU_VOBJECTS);
							ShowDialog(playerid, dialogid);
							return 1;
						}
						if(slot == -1)
						...
						format(VEHICLE_OBJECTS[ PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID] ][slot][vobject_text_TEXT], 32, "Mi texto");
					...
					case 3:
					{
						...
						{
							PlayerPlaySoundEx(playerid, 1085, 0.0, 0.0, 0.0);
							SendClientMessageEx(playerid, COLOR_WHITE, ""COL_WHITE"¡Los jugadores con una{b1fc6f}Tarjeta VIP "COL_WHITE"pueden tener hasta %d objetos en vehículos! Usa {fffb49}/ayuda "COL_WHITE"si quieres ser tener una{b1fc6f}Tarjeta VIP.", MAX_SU_VOBJECTS);
							ShowDialog(playerid, dialogid);
							return 1;
						}
						if(slot == -1)
			...
			else
			{
				...
				GLOBAL_VEHICLES[ PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID] ][gb_vehicle_PARAMS_ENGINE] = 1;
				UpdateVehicleParams(PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID]);

				if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) PutPlayerInVehicleEx(playerid, PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID], 0);
		...
		case DIALOG_VOBJECT_OBJECT_COLOR:
		{
			if(response)
			{
				switch(listitem)
				{
					...
					case 1:
					{
						VEHICLE_OBJECTS[ PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID] ][ PLAYER_TEMP[playerid][py_TUNING_EDIT_SLOT] ][vobject_COLORS][ PLAYER_TEMP[playerid][py_TUNING_EDIT_COLOR_SLOT] ] = RGBAToARGB(VEHICLE_COLORS[ GLOBAL_VEHICLES[ PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID] ][gb_vehicle_COLOR_1] ]);
						UpdateVehicleAttachedObject(PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID], PLAYER_TEMP[playerid][py_TUNING_EDIT_SLOT], true);
						ShowPlayerMessage(playerid, "Color actualizado.", 2);
						ShowDialog(playerid, DIALOG_VOBJECT_OBJECT_COLORS);
					}
					case 2:
					{
						VEHICLE_OBJECTS[ PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID] ][ PLAYER_TEMP[playerid][py_TUNING_EDIT_SLOT] ][vobject_COLORS][ PLAYER_TEMP[playerid][py_TUNING_EDIT_COLOR_SLOT] ] = RGBAToARGB(VEHICLE_COLORS[ GLOBAL_VEHICLES[ PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID] ][gb_vehicle_COLOR_2] ]);
						UpdateVehicleAttachedObject(PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID], PLAYER_TEMP[playerid][py_TUNING_EDIT_SLOT], true);
						ShowPlayerMessage(playerid, "Color actualizado.", 2);
						ShowDialog(playerid, DIALOG_VOBJECT_OBJECT_COLORS);
					}
				...
				switch(listitem)
				{
					...
					case 3:
					{
						if(VEHICLE_OBJECTS[ PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID] ][ PLAYER_TEMP[playerid][py_TUNING_EDIT_SLOT] ][vobject_text_BOLD])
						{
					        ...
					        ShowPlayerMessage(playerid, "Negrita desabilitada.", 2);
						}
						else
						{
							...
							ShowPlayerMessage(playerid, "Negrita habilitada", 2);
						}
						UpdateVehicleAttachedObject(PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID], PLAYER_TEMP[playerid][py_TUNING_EDIT_SLOT], true);
						ShowDialog(playerid, dialogid);
					}
		...
		case DIALOG_VOBJECT_TEXT_FONT_COLOR:
		{
			if(response)
			{
				VEHICLE_OBJECTS[ PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID] ][ PLAYER_TEMP[playerid][py_TUNING_EDIT_SLOT] ][vobject_text_FONT_COLOR] = RGBAToARGB(RandomColors[listitem]);
				UpdateVehicleAttachedObject(PLAYER_TEMP[playerid][py_TUNING_GARAGE_VEHICLEID], PLAYER_TEMP[playerid][py_TUNING_EDIT_SLOT], true);
		...
		case DIALOG_PLAYER_CONFIG:
		{
			if(response)
			{
				switch(listitem)
				{
					...
					case 3:
					{
						if(PLAYER_MISC[playerid][MISC_CONFIG_LOWPC] == true)
						{
						...
						{
							PLAYER_MISC[playerid][MISC_CONFIG_LOWPC] = true;
