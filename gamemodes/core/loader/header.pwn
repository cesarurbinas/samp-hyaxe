...
stock LoadStaticVehiclesFromFile(const filename[])
{
	...
	while(fread(file_ptr,line,256) > 0)
	{
		...
		new vid = AddWorldVehicle(true, vehicletype, SpawnX, SpawnY, SpawnZ, SpawnRot, Color1, Color2, (30*60));
		
		format(GLOBAL_VEHICLES[vid][gb_vehicle_NUMBER_PLATE], 32, "hy%d", vid);
		SetVehicleNumberPlate(vid, GLOBAL_VEHICLES[vid][gb_vehicle_NUMBER_PLATE]);
		
		vehicles_loaded ++;
	}

	fclose(file_ptr);
	printf("[debug] Loaded %d vehicles from: %s", vehicles_loaded, filename);
...
SanAndreas()
{
    ...
    printf("[debug] Total vehicles from files: %d", total_vehicles_from_files);
	return 1;
}