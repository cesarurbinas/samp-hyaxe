		...
		for(new i; i < db_num_rows(Result); i++ )
		{
			...
			else
			{
				...
				if (db_num_rows(Result_info_owner))
				{
					...
					format(label_str, 164, ""COL_WHITE"%s (%s)\nEntrada: %s\nPropietario:{35A7FF} %s", CLUBS_INFO[i][club_NAME], (CLUBS_INFO[i][club_ENTER_PRICE] ? ""COL_GREEN"Abierto"COL_WHITE"" : sprintf(""COL_YELLOW"%d$"COL_WHITE"", CLUBS_INFO[i][club_ENTER_PRICE])), (CLUBS_INFO[i][club_STATE] ? ""COL_GREEN"Gratis"COL_WHITE"" : ""COL_RED"Cerrado"COL_WHITE""), owner);
				}
			}
