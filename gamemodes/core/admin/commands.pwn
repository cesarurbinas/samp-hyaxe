...
CMD:reportmute(playerid, params[])
{
	...
	new str[145];
	format(str, 145, "[ADMIN] %s (%d) le bloqueó los reportes a %s (%d)", ACCOUNT_INFO[playerid][ac_NAME], playerid, ACCOUNT_INFO[to_player][ac_NAME], to_player);
	SendMessageToAdmins(COLOR_ANTICHEAT, str);
	SendDiscordWebhook(str, 1);
	return 1;
}
...
CMD:reportunmute(playerid, params[])
{
	...
	new str[145];
	format(str, 145, "[ADMIN] %s (%d) le desbloqueó los reportes a %s (%d)", ACCOUNT_INFO[playerid][ac_NAME], playerid, ACCOUNT_INFO[to_player][ac_NAME], to_player);
	SendMessageToAdmins(COLOR_ANTICHEAT, str);
	SendDiscordWebhook(str, 1);
	return 1;
}
